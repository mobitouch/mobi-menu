...
app.use(
  session({
    secret: process.env.SESSION_SECRET || "lorem-ipsum-secret-key-2025-change-in-production",
    resave: false,
...
if (!ADMIN_PASSWORD) {
  ...
  console.error("⚠️  Please create a .env file with ADMIN_PASSWORD=your_password");
  process.exit(1);
}
  ...
  try {
    ...
    if (settings.filterCategories && Array.isArray(settings.filterCategories)) {
      filterCategories = settings.filterCategories
        ...
        .filter(f => f.category && f.label && f.category.length <= 100 && f.label.length <= 100);
    }
